...
class CreatePrimMap {
    ...
    void SetProperty(int rows, int cols, List<String> wallList, MazeGrid grid[][]) {
        this.rows = rows;
        this.cols = cols;
...
//        this.visitedset = visited;
    ...
    private MazeGrid[][] createMap(MazeGrid[][] mazeGrid) {
        ...
        while (!wallList.isEmpty()) {
            ...
            //将墙两边的格子放入通路集合
            if (((ajX - 1) >= 0) && mazeGrid[ajX - 1][ajY].isMark() && mazeGrid[ajX - 1][ajY].noVisited()) {
                mazeGrid[ajX][ajY].setMark();              //将当前墙设为通路
                mazeGrid[ajX - 1][ajY].setVisited();       //将墙两边的通路格子纳入通路集合
                visitX = ajX - 1;                              //visitX为通路块坐标
            } else if (((ajX + 1) < cols) && mazeGrid[ajX + 1][ajY].isMark() && mazeGrid[ajX + 1][ajY].noVisited()) {
                mazeGrid[ajX][ajY].setMark();              //将当前墙设为通路
                mazeGrid[ajX + 1][ajY].setVisited();       //将墙两边的通路格子纳入通路集合
                visitX = ajX + 1;
            } else if (((ajY - 1) >= 0) && mazeGrid[ajX][ajY - 1].isMark() && mazeGrid[ajX][ajY - 1].noVisited()) {
                mazeGrid[ajX][ajY].setMark();              //将当前墙设为通路
                mazeGrid[ajX][ajY - 1].setVisited();       //将墙两边的通路格子纳入通路集合
                visitY = ajY - 1;
            } else if (((ajY + 1) < rows) && mazeGrid[ajX][ajY + 1].isMark() && mazeGrid[ajX][ajY + 1].noVisited()) {
                mazeGrid[ajX][ajY].setMark();              //将当前墙设为通路
                mazeGrid[ajX][ajY + 1].setVisited();       //将墙两边的通路格子纳入通路集合
                visitY = ajY + 1;
            }
